...
CyberAgent is an AI-native task management platform — like Linear, but with AI agents as first-class citizens.
...
make cli ARGS="..."   # Run cyberagent CLI (e.g. make cli ARGS="config")
make test             # Go tests
...
pnpm --filter @cyberagent/views exec vitest run auth/login-page.test.tsx
pnpm --filter @cyberagent/core exec vitest run runtimes/version.test.ts
pnpm --filter @cyberagent/web exec vitest run app/\(auth\)/login/page.test.tsx
...
pnpm --filter @cyberagent/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @cyberagent/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@cyberagent/core` imports (pure UI, no business logic).
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@cyberagent/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@cyberagent/views`, the test is in the wrong place — move it to `packages/views/` and mock `@cyberagent/core` instead.
...
- Mock `@cyberagent/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@cyberagent/core/api` for API calls.
