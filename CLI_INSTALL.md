# CyberAgent CLI — Installation Guide for AI Agents
...
```
Fetch https://github.com/DrOlu/CyberAgent/blob/main/CLI_INSTALL.md and follow the instructions to install CyberAgent CLI, log in, and start the daemon on this machine.
```

---

## Step 1: Check if CyberAgent CLI is already installed
...
## Step 2: Install the CyberAgent CLI
...
LATEST=$(curl -sI https://github.com/DrOlu/CyberAgent/releases/latest | grep -i '^location:' | sed 's/.*tag\///' | tr -d '\r\n')

# Download and extract
VERSION="${LATEST#v}"
curl -sL "https://github.com/DrOlu/CyberAgent/releases/download/${LATEST}/multica-cli-${VERSION}-${OS}-${ARCH}.tar.gz" -o /tmp/multica.tar.gz
...
> "A browser window will open for CyberAgent login. Please complete the authentication in your browser, then come back here."
...
> "The CyberAgent daemon is running but no AI agent CLIs were detected. Please install at least one supported CLI (`claude`, `codex`, `copilot`, `opencode`, `openclaw`, `hermes`, `gemini`, `pi`, or `cursor-agent`), then restart the daemon with `multica daemon stop && multica daemon start`."
...
> "CyberAgent CLI is installed and the daemon is running. Agents in your workspaces can now execute tasks on this machine. You can manage workspaces with `multica workspace list` and view daemon logs with `multica daemon logs -f`."
