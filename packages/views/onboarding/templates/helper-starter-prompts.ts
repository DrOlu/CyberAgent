...
export const HELPER_STARTER_PROMPTS: Record<StarterCardId, StarterPrompt> = {
  ...
  tour: {
    ...
    prompt: {
      ...
      zh: "陪我熟悉 CyberAgent 的每个核心功能 —— 任务、agent、squad、autopilot、chat。挑一个我可能用得上的真实场景,讲讲这几个东西是怎么配合的。",
      ko: "Multica의 핵심 기능인 태스크, agent, squad, autopilot, chat을 안내해 주세요. 제가 실제로 겪을 만한 상황 하나를 골라 이 요소들이 어떻게 함께 작동하는지 설명해 주세요.",
      ja: "Multica の主要な機能 — タスク、agent、squad、autopilot、chat を案内してください。私が実際に遭遇しそうな現実的なシナリオを1つ選び、これらの要素がどう連携するのかを説明してください。",
    },
  },
  welcome_page: {
    title: {
...
When done, also reply with a one-sentence summary of which scenarios you picked for me and why.`,
      zh: `给我做一份单文件 HTML 演示稿,介绍 CyberAgent 能为我做什么。根据我的角色和使用场景定制(见下面"关于我")。把完整 HTML 贴到这条任务的评论里的 \`\`\`html 代码块中,我直接复制下来存成 \`multica-intro.html\` 双击就能在浏览器里打开。
...
做完后再用一句话告诉我你为我挑了哪几个场景以及为什么。`,
      ko: `Multica가 저에게 무엇을 해줄 수 있는지 보여주는 단일 파일 HTML 슬라이드 덱을 만들어 주세요. 제 역할과 사용 사례에 맞춰 주세요(아래 "내 정보" 참고). 전체 HTML을 이 태스크의 댓글에 fenced \`\`\`html 코드 블록으로 붙여 주세요. 그대로 복사해 \`multica-intro.html\`로 저장하고 브라우저에서 더블클릭해 열 수 있어야 합니다.
...
완료 후, 어떤 시나리오를 골랐고 왜 골랐는지 한 문장으로 요약해 주세요.`,
      ja: `Multica が私に何をしてくれるのかを示す、単一ファイルの HTML スライドデッキを作ってください。私の役割とユースケースに合わせてください(下の「私について」を参照)。完全な HTML を、このタスクのコメントに fenced \`\`\`html コードブロックで貼り付けてください。そのままコピーして \`multica-intro.html\` として保存し、ブラウザでダブルクリックして開けるようにしてください。
