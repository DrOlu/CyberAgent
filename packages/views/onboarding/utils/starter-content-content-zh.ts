...
export const PROJECT = {
  title: "上手指南",
  description: "几件可以在 CyberAgent 里上手试一试的事，按你的节奏走。",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    `欢迎来到 CyberAgent！👋`,
    ``,
    `这是你工作区里的第一个 issue。下面你的智能体马上会回复——这就是 CyberAgent 里工作的方式：你写下你想做的事，智能体（或同事）接手并在评论里回复。`,
    ``,
    `[了解 CyberAgent 是怎么运转的 →](https://multica.ai/docs/zh/how-multica-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  switch (q.use_case) {
    case "ship_code":
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${name} 主要会让你做 **编码相关的工作**。\n\n${sharedInstructions}`,
      };
    case "manage_team":
    case "plan_research":
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${name} 希望你帮忙做 **规划与拆解工作**。\n\n${sharedInstructions}`,
      };
    case "write_publish":
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${name} 会让你做 **写作、编辑、发布** —— 起草、摘要、分析。\n\n${sharedInstructions}`,
      };
    case "evaluate":
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${name} 还在 **探索** CyberAgent 能做什么 —— 暂时没有具体目标。\n\n${exploreInstructions}`,
      };
    case "personal_tasks":
    case "automate_ops":
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${name} 希望你帮忙做 **日常任务和自动化**。\n\n${sharedInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 欢迎来到 CyberAgent —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 CyberAgent。\n\n${sharedInstructions}`,
      };
  }
}
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**每个 issue 右侧都有一个 Properties 面板**。从这里控制谁来做什么。CyberAgent 里的智能体被触发的条件是 issue 同时满足：`,
        ``,
