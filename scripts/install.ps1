# CyberAgent installer for Windows — one command to get started.
#
...
#
# Self-host: starts a local CyberAgent server + installs CLI + configures
#   $env:MULTICA_MODE="local"; irm https://raw.githubusercontent.com/DrOlu/CyberAgent/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/DrOlu/CyberAgent.git"
$RepoWebUrl    = "https://github.com/DrOlu/CyberAgent"
...
function Install-CliBinary {
    Write-Info "Installing CyberAgent CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "CyberAgent requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/DrOlu/CyberAgent/releases/download/$latest/multica-cli-$version-windows-$arch.zip"
    ...
    $checksumUrl = "https://github.com/DrOlu/CyberAgent/releases/download/$latest/checksums.txt"
    try {
    ...
    Write-Ok "CyberAgent CLI installed to $binDir\multica.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "multica") {
        ...
        if ($isUpToDate) {
            Write-Ok "CyberAgent CLI is up to date ($currentVer)"
            return
        }

        Write-Info "CyberAgent CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "CyberAgent CLI upgraded ($currentVer -> $newVer)"
        return
    }
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. CyberAgent self-hosting requires Docker and Docker Compose.
...
function Install-Server {
    Write-Info "Setting up CyberAgent server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning CyberAgent repository..."
    ...
    Write-Info "Pulling official CyberAgent images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting CyberAgent services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "CyberAgent server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  CyberAgent - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] CyberAgent CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     multica setup               " -NoNewline; Write-Host "# Connect to CyberAgent Cloud (multica.ai)" -ForegroundColor DarkGray
    ...
    Write-Host '     $env:MULTICA_MODE="with-server"; irm https://raw.githubusercontent.com/DrOlu/CyberAgent/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  CyberAgent - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host '     $env:MULTICA_MODE="stop"; irm https://raw.githubusercontent.com/DrOlu/CyberAgent/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping CyberAgent services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No CyberAgent installation found at $InstallDir"
    }
