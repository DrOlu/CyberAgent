# CyberAgent Docs 站重写规划（v2）

> **本规划是什么**：CyberAgent 对外 doc 站（`apps/docs/`，Fumadocs + Next.js）的从零重写方案。它替换 v1 规划——v1 之前在代码调研之前写的，很多对概念的切分现在看是错的。
>
...
>
> **风格目标**：排版/布局对标 Anthropic docs（奶油底、serif heading、宽松行距、窄行宽、深色代码块的灵魂），但**色板继续用 CyberAgent 自己的 tokens**（冷蓝 brand）——visual 上是"CyberAgent 色 + Anthropic 排版语法"。

---

## 一、产品定位（文档的落脚点）

CyberAgent = **人 + AI agent 在同一个看板上协作的任务管理平台**。
...
> **关键**：P2 的 agent 不会逛导航，只会被人类用 `Fetch https://docs.cyberagent.ai/...` 指向某一页。所以每一页都要**自包含**。
...
| **How CyberAgent works** | 一张大图把 User / Issue / Agent / Runtime / Daemon / Skill / Task / Trigger 之间的关系串起来——目标是建立**正确心智模型**，而不是记名词 |

### 板块 2. Getting Started（3 篇）

| 篇目 | 核心内容 |
|---|---|
| **Cloud Quickstart** | 5 分钟：signup → install CLI → `cyberagent setup` → 第一个 agent → 第一个 issue |
| **Self-Host Quickstart** | 10 分钟：`install.sh --with-server` → `cyberagent setup self-host` |
...
- `apps/docs/app/global.css` 里 `@import "@cyberagent/ui/styles/tokens.css"`，覆盖 Fumadocs 的 `neutral.css`
...
当前 `.github/workflows/ci.yml:33` 用 `--filter='!@cyberagent/docs'` 排除了 docs build。**在 Phase 0 做一个独立小 PR 把它加回来**——否则 MDX 语法错 CI 不拦，只有 Vercel 部署时才发现。
