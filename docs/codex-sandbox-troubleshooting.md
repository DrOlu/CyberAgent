...
[mul-963]: https://cyberagent-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://cyberagent-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *cyberagent-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN cyberagent-managed` /
`# END cyberagent-managed` markers. Anything outside the markers is left
...
`[permissions.cyberagent]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.cyberagent.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the CyberAgent API reachable at all?
curl -sSf https://cyberagent-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
cyberagent issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.cyberagent`
  profile that allows only `cyberagent-api.copilothub.ai` and
  `cyberagent-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN cyberagent-managed/,/# END cyberagent-managed/p' \
  ~/cyberagent_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
