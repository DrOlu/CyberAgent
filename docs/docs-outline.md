...
5. **服务于产品定位**：CyberAgent 的核心差异化是 **"BYO-agent 的 Linear—— agent 跑在你自己的机器，你掌控计算和 provider 选择"**。每篇的语气和深度都应该为这个叙事服务。
...
---
title: Issues
description: Issue 是 CyberAgent 的核心工作对象——人和 agent 都能被分配、评论、改状态。
---
...
Issue 是 CyberAgent 的核心工作对象……（1-2 段话说清楚"是什么"）

## 关键概念

### Polymorphic Assignee

Issue 的 assignee 可以是 member（人）或 agent。这是 CyberAgent 和传统 task manager
...
```bash
cyberagent issue assign <issue-id> --agent <agent-slug>
```

分配给 agent 时，CyberAgent 会立刻入队一个 task 到对应 runtime。详见
...
- **目标读者**: P0 新用户 / evaluator（第一次听说 CyberAgent）
...
### 1.2 How CyberAgent Works — ⬜ Not started [v1]

- **Source files**: `server/cmd/cyberagent-daemon/`, `packages/core/`, 战略 plan 的"产品定位"段
...
- **Source files**: `apps/docs/content/docs/cloud-quickstart.mdx`（现有）, `server/cmd/cyberagent/cmd_setup.go`, `cmd_login.go`, `cmd_agent.go`, `cmd_issue.go`
- **目标读者**: P0 新用户（想 5 分钟跑起来）
- **叙事位置**: 第三页。
- **写什么**（600-1000 字）:
  - Signup → install CLI → `cyberagent login` → `cyberagent setup cloud` → 创建第一个 agent → 创建第一个 issue → 分配给 agent → 看它工作
  - 每步命令可独立复制运行
...
- **Source files**: `Makefile`（selfhost target）, `docker-compose.selfhost.yml`, `.env.example`, `server/cmd/cyberagent/cmd_setup.go`
...
- **写什么**（800-1200 字）:
  ...
  - 第一次启动后 `cyberagent setup self-host`
  - 最小可行配置（必填 env）
...
- **Source files**: `server/internal/handler/agent.go`, `server/pkg/db/queries/agent.sql`, `packages/views/agents/`, `server/cmd/cyberagent/cmd_agent.go`
- **目标读者**: P1 团队管理员
- **叙事位置**: 怎么创建一个 agent。
- **写什么**（1200-1800 字）:
...
- **Source files**: `server/internal/handler/skill.go`, `server/pkg/db/queries/skill.sql`, `server/internal/daemon/execenv/context.go`（各 provider skill 注入路径）, `server/cmd/cyberagent/cmd_skill.go`
...
- **写什么**（1200-1800 字）:
  ...
  - **兼容性宣示**：
    > "CyberAgent Skill 采用 [Anthropic Agent Skills 开放标准](https://agentskills.io) 的 `SKILL.md` 格式。所有符合该规范的 skill（包括 Anthropic 官方仓库、ClawHub、skills.sh 上发布的包）都可以直接导入使用。"
  ...
  - **末尾一段"Skills vs MCP"**（v2 再开 MCP 独立页）:
    > MCP（Model Context Protocol）是另一层概念——让 agent 连外部工具（数据库、文件系统、第三方 API）。CyberAgent 支持 `mcp_config` 字段，但目前**仅 Claude Code 真实消费**，其他 provider 接收但未传递。详见 v2 的 MCP 专页（开发中）。
...
> **板块叙事**：agent 怎么真的动起来——分布式执行、用户自己跑 daemon。这是 CyberAgent 结构上区别于 Linear/Jira 的关键部分。
...
- **Source files**:
  - Daemon: `server/internal/daemon/daemon.go`, `server/cmd/cyberagent-daemon/main.go`, `server/cmd/cyberagent/cmd_daemon.go`, `server/pkg/db/queries/daemon.sql`
...
- **写什么**（1500-2000 字）:
  - **Daemon 部分**:
    ...
    - CLI：`cyberagent daemon install/login/start/stop/status/logs`
  - **Runtime 部分**:
...
- **写什么**（1500-2500 字）:
  ...
  - 每个 provider 一小段（80-150 字）：核心定位 + 用户画像 + 官网链接 + CyberAgent 兼容性
  - **Session resume 精确现状**:
...
- **写什么**（600-1000 字）:
  ...
  - CLI：`cyberagent issue assign <id> --agent <agent-slug>`
  - 分配后立刻入队 task
...
- **Source files**: `server/cmd/cyberagent/main.go`（命令树）, 各 `cmd_*.go`
- **目标读者**: P1 开发者、P2 agent
- **叙事位置**: Reference 板块首篇。
- **写什么**（2000-2500 字）:
  - **认证入口**（开头一段）:
    - `cyberagent login` → 拿 PAT（`mul_` 前缀）
    - PAT 存 `~/.cyberagent/config.json`
...
- **Source files**: `server/internal/handler/auth.go`, `server/internal/middleware/auth.go`, `server/internal/middleware/daemon_auth.go`, `server/cmd/cyberagent/cmd_auth.go`
- **目标读者**: P1 管理员、P1 开发者（用 API / CLI / daemon）
- **叙事位置**: Reference 板块。讲"三种身份证"。
- **写什么**（1200-1800 字）:
  - **3 种 token**:
    - **JWT Cookie**（`cyberagent_auth`，HttpOnly，30 天）—— 浏览器
    - **PAT**（`mul_` 前缀）—— CLI / 脚本
  ...
  - PAT 创建 / 撤销 / 管理（UI 在 Settings，CLI 通过 `cyberagent login`）
  - Daemon token 生成时机（`cyberagent daemon login`）
...
- **写什么**（1000-1500 字）:
  ...
  - **Daemon 不内置**：desktop 只是窗口，daemon 要单独 `cyberagent daemon start`，desktop package 里 bundle 了 CLI
...
- **板块 8**（CLI 详细 reference，从 8.1 cheatsheet 拆开）:
  - 8.4 cyberagent auth / login 详细
  - 8.5 cyberagent setup 详细
  - 8.6 cyberagent workspace 详细
  - 8.7 cyberagent issue（+comment / subscriber）详细
  - 8.8 cyberagent project 详细
  - 8.9 cyberagent agent（+skills）详细
  - 8.10 cyberagent skill 详细
  - 8.11 cyberagent autopilot 详细
  - 8.12 cyberagent repo 详细
  - 8.13 cyberagent daemon 详细
  - 8.14 cyberagent runtime 详细
  - 8.15 cyberagent config / version / update / attachment 详细
