...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- `multica issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>] [--no-start]` — update fields; pass `--parent \"\"` to clear parent. `--no-start` applies ownership/status changes without starting another agent run.\n")
	...
	b.WriteString("- `multica issue assign <id> (--to X | --to-id <uuid> | --unassign) [--no-start]` — change ownership. Use `--no-start` when claiming an issue from work already underway so the ownership write does not launch a duplicate run.\n")
	b.WriteString("- `multica issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled). Use `--no-start` when the status write belongs to work already underway in another run.\n")
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("4. Complete the task within your Agent Identity boundaries (`## Instruction Precedence` lists the actions Agent Identity can forbid). If your role is delegation-only, perform the allowed delegation work and stop once that outcome is delivered. Before assigning an issue to yourself, check the target issue's comment history for an existing claim and inspect any `## Active sibling runs` with the concrete `run-messages` commands it provides. If assignment or status only records ownership/progress for work already being done, pass `--no-start` on every such command; keep the default start behavior for an intentional handoff to fresh work.\n")
	if ctx.IsSquadLeader {
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("Fetch issue/comment attachments via the authenticated CLI (`multica attachment --help`); never open CyberAgent resource URLs directly.\n")
...
func writeAlwaysUseCLI(b *strings.Builder) {
	...
	b.WriteString("Access CyberAgent platform resources only through the `multica` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
