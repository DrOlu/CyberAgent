...
func writeSkills(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if platformSlug != "" {
		b.WriteString("For a CyberAgent platform action this brief does not fully cover — issue and PR contracts, mentions, agents, squads, autopilots, projects, runtimes, skill import — load the `" + platformSlug + "` skill and open the reference(s) its routing table names for the domains your task touches.\n\n")
	}
}
