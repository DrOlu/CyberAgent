...
// treats a hook as CyberAgent-owned if its content contains ANY of these
...
var daemonInstalledHookSignatures = []string{
	multicaHookMarker,
	"# Installed by the CyberAgent daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the CyberAgent Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# multica:prepare-commit-msg:co-authored-by
# CyberAgent: add Co-authored-by trailer for the CyberAgent Agent.
...
TRAILER="Co-authored-by: multica-agent <github@cyberagent.sh>"
...
// a Co-authored-by trailer for the CyberAgent Agent. The hook is installed in the
...
// installed by the CyberAgent daemon (current or any previously released
