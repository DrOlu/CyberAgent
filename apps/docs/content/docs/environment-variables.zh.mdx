...
| `MULTICA_SHUTDOWN_HOLD_DURATION` | `0` | 否。收到 `SIGTERM`/`SIGINT` 后、开始优雅退出前的可选等待时间。接受 `300s`、`5m` 等非负 Go duration；留空或设为 `0` 时立即开始退出 |

在 Kubernetes 中使用 `MULTICA_SHUTDOWN_HOLD_DURATION` 时，`terminationGracePeriodSeconds` 必须大于 hold 时长与后续优雅退出预算之和。如果两者配置成相同时长，kubelet 可能在清理完成前发送 `SIGKILL`。
