...
This shape is intentional — it's the same pattern CyberAgent already uses for agent providers: a `type` discriminator and a typed payload. It keeps the schema stable so adding "Notion page", "Google Doc", "uploaded file", or "external URL" later is a small, additive change.
...
The trade-off vs. `github_repo` is intentional: only the bound daemon can pick up tasks against the directory, and tasks on the same directory run **serially** instead of in parallel. In exchange you keep your existing checkout, your existing branch, your existing dirty state — CyberAgent never re-clones it.
...
**The default answer is `github_repo`.** Only these cases justify switching to `local_directory`:

1. **Re-cloning is prohibitively expensive** — a multi-gigabyte game checkout, a monorepo with heavy LFS assets, or anything where the per-task `git clone` would dominate the actual work. This is the mode's primary reason to exist.
2. **The directory isn't a git repo at all** — an art asset library, a docs folder, an exported dataset. There's no remote to clone from, so working in place is the only option.
3. **The work depends on machine-local state** — a local database, a multi-gigabyte dataset, an engine / SDK / licensed toolchain installed on that machine, attached hardware. None of it exists inside a fresh worktree.

These, on the other hand, are **not** reasons to pick it:

- **"I want to see the agent's edits in my own editor."** Worktrees are on your local disk too (under `~/multica_workspaces/`) and open in any editor. Giving up concurrency to save one directory hop isn't a good trade.
- **"Cloning is a bit slow."** It only counts as a cost when the clone outweighs the task itself. A few-hundred-megabyte repo doesn't qualify.
- **"I don't want agent branches piling up."** Branch policy belongs in `default_branch_hint` and task conventions, not in your choice of execution mode.

**Serial execution is a premise of this mode, not a limitation waiting to be fixed.** This version ships no file-level write lock, so the per-directory serial gate (one task at a time on the same folder) is the only thing stopping agents in two different issues from writing the same files at once. That's also why "add worktrees to `local_directory` so tasks can run in parallel" is self-contradictory — a worktree *is* another copy of the working tree, and you chose this mode precisely because copies are too expensive. **If you need real parallelism on the same codebase, the answer is `github_repo`.**

### Attaching a local directory

<Callout type="info">
Before you do this, make sure you actually need it (see the section above). If the folder is an ordinary git repo you can clone, `github_repo` is the better choice — its concurrency is unbounded. `local_directory` targets cases like a tens-of-gigabytes game project, where re-cloning once is already too expensive.
</Callout>
...
The blacklist is intentionally aggressive — picking your home directory would put CyberAgent's runtime files at the root of your account, which is never what you want. Pick a sub-folder (typically your actual project checkout) instead.
...
4. Keeps CyberAgent's runtime artefacts (`output/`, `logs/`, `.gc_meta.json`) in a separate envRoot **outside** the user's directory.
...
### What CyberAgent will and won't touch in your directory
...
multica project create \
  --title "Agent UX 2026" \
  --repo https://github.com/DrOlu/CyberAgent
  ...
  "resources": [
    {
      ...
      "resource_type": "github_repo",
      "resource_ref": {
        "url": "https://github.com/DrOlu/CyberAgent",
        "default_branch_hint": "main"
      }
    }
  ]
...
- **GitHub repo**: https://github.com/DrOlu/CyberAgent (default branch: `main`)
