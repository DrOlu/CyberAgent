...
description: What a CyberAgent task can reach on the machine that runs it, and where the real isolation boundary lives.
---
...
CyberAgent does not sandbox the filesystem for you. If the daemon runs as your personal user account, a task can read your SSH keys, edit your shell profile, and delete your documents. Isolation has to come from the boundary you put the daemon in.
</Callout>
...
So CyberAgent does not pretend to be the boundary. Put one around it.
...
- **Task-scoped API tokens.** The `MULTICA_TOKEN` handed to a task is bound to that agent and that task by the server, so a task cannot act as you or as another agent through the CyberAgent API.

## What is not a boundary

- **The coding tool's own sandbox and approval settings.** CyberAgent runs agents unattended, so approval prompts are answered automatically. On the default path the filesystem sandbox is off as well: Codex runs with `sandbox_mode = "danger-full-access"` and Claude Code with `--permission-mode bypassPermissions`. The exception is Windows — if you explicitly configure Codex's native sandbox (`windows.sandbox = "unelevated"` or `"elevated"`), CyberAgent honors that opt-in and keeps `workspace-write` for those tasks. Worth doing where it fits, but it is one tool on one platform and it does not change how you should scope the daemon user.
