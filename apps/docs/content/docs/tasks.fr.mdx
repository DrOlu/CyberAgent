...
Chaque fois qu'un agent commence à travailler, CyberAgent crée une **exécution**. Elle enregistre ce qui a déclenché le travail, à quel agent il a été confié, où il en est et s'il a finalement réussi.
...
Les points d'entrée fournissent des contextes différents, mais l'exécution fonctionne toujours de la même façon : CyberAgent crée une exécution, un runtime la prend en charge, et le runtime invoque l'outil de codage IA configuré pour l'agent.
