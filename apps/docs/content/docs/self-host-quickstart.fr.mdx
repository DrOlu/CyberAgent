...
description: Démarrez CyberAgent avec Docker Compose, connectez-vous, puis reliez votre premier ordinateur.
---

import { Callout } from "fumadocs-ui/components/callout";

L'auto-hébergement de CyberAgent comporte deux parties :

| Partie | Ce qu'elle exécute | Où elle se trouve |
|---|---|---|
| **Service CyberAgent** | Web, API et PostgreSQL | Une machine sur laquelle Docker est installé |
| **Ordinateur** | Le daemon CyberAgent et les outils de codage IA | L'ordinateur sur lequel les développeurs travaillent réellement |

Il peut s'agir de la même machine ou de machines distinctes. L'auto-hébergement ne remplace que la partie CyberAgent Cloud.
...
La machine qui exécute le service CyberAgent a besoin de :
...
CyberAgent utilise Compose v2, invoqué via `docker compose`. L'ancien `docker-compose` v1 n'est pas pris en charge.

L'ordinateur doit également disposer d'au moins un [outil de codage IA](/providers) installé et connecté, comme Claude Code, Codex ou Cursor. Le CLI CyberAgent est installé à l'étape 5.

## 1. Démarrer CyberAgent
...
3. Télécharge les images PostgreSQL, backend CyberAgent et frontend CyberAgent
...
- `app.example.com` : l'application web CyberAgent
...
Installez d'abord le CLI CyberAgent :
...
Si le service CyberAgent tourne sur ce même ordinateur :

```bash
multica setup self-host
```

Si le service CyberAgent tourne sur une autre machine, passez les deux URL configurées précédemment :
...
De retour dans CyberAgent, dès qu'un runtime en ligne apparaît dans la liste des runtimes, créez un agent et assignez-lui votre première tâche.
...
`git pull` met à jour **`docker-compose.selfhost.yml` lui-même** — nouvelles variables d'environnement, nouveaux services, vérifications d'état modifiées. Ce n'est **pas** ainsi que vous obtenez une nouvelle version de CyberAgent.
