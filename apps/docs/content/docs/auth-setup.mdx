...
2. Set the **Authorized redirect URIs** to your CyberAgent frontend address plus `/auth/callback`, for example:
...
**Only people who already have a CyberAgent account.** Accepting an invite doesn't check the signup allowlist — if the invitee has signed up already (for example in another workspace), clicking the invite link and signing in lets them accept.
