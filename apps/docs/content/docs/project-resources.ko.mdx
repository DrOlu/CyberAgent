...
이 형태는 의도적인 것입니다 — CyberAgent가 에이전트 제공자에 이미 사용하는 것과 동일한 패턴입니다: `type` 판별자 하나와 타입이 지정된 페이로드. 스키마를 안정적으로 유지하므로, 추후 "Notion 페이지", "Google Doc", "업로드된 파일", "외부 URL"을 추가하는 것은 작고 점진적인 변경이 됩니다.
...
대상 디렉터리가 평범하게 클론할 수 있는 일반 git 저장소라면 **대신 `github_repo`를 사용하세요**: 기본적으로 워크트리 모드로 동작하므로 같은 저장소에 대한 작업을 무제한으로 동시에 실행할 수 있습니다. `local_directory`를 선택한다는 것은 그 폴더의 작업이 이후로 **항상 한 번에 하나씩 직렬로** 실행된다는 뜻입니다. 결정하기 전에 아래 "선택할 때"를 읽어보세요.
</Callout>
...
`github_repo`와 비교한 트레이드오프는 의도적입니다: 바인딩된 데몬만 해당 디렉터리에 대한 작업을 가져갈 수 있고, 같은 디렉터리에 대한 작업은 병렬이 아니라 **직렬로** 실행됩니다. 그 대가로 기존 체크아웃, 기존 브랜치, 기존 더티 상태를 그대로 유지합니다 — CyberAgent는 절대 다시 클론하지 않습니다.
...
**기본 답은 `github_repo`입니다.** `local_directory`로 전환할 만한 경우는 다음뿐입니다:

1. **다시 클론하는 비용이 지나치게 큰 경우** — 수 기가바이트짜리 게임 체크아웃, 무거운 LFS 자산을 가진 monorepo, 또는 작업당 `git clone`이 실제 작업을 압도하는 모든 경우. 이 모드가 존재하는 가장 큰 이유입니다.
2. **그 디렉터리가 애초에 git 저장소가 아닌 경우** — 아트 에셋 라이브러리, 문서 폴더, 내보낸 데이터셋 등. 클론할 원격이 없으므로 제자리에서 작업하는 수밖에 없습니다.
3. **작업이 그 기기의 로컬 상태에 의존하는 경우** — 로컬 데이터베이스, 수십 기가바이트짜리 데이터셋, 그 기기에 설치된 엔진 / SDK / 라이선스가 필요한 툴체인, 연결된 하드웨어. 새로 만든 워크트리 안에는 어느 것도 존재하지 않습니다.

반대로, 다음은 이 모드를 선택할 이유가 **되지 않습니다**:

- **"에이전트가 무엇을 고쳤는지 내 에디터에서 보고 싶다."** 워크트리도 똑같이 로컬 디스크(`~/multica_workspaces/` 아래)에 있고 어떤 에디터로든 열 수 있습니다. 디렉터리 한 단계를 덜 거치려고 동시성을 포기하는 것은 남는 장사가 아닙니다.
- **"클론이 조금 느리다."** 클론이 작업 자체를 압도할 때에야 비용이라고 할 수 있습니다. 수백 메가바이트짜리 저장소는 여기 해당하지 않습니다.
- **"에이전트 브랜치가 쌓이는 게 싫다."** 브랜치 정책은 `default_branch_hint`와 작업 차원의 합의로 풀어야지, 실행 모드 선택으로 풀 문제가 아닙니다.

**직렬 실행은 이 모드의 전제이지, 언젠가 고쳐질 제한이 아닙니다.** 이 버전은 파일 단위 쓰기 잠금을 제공하지 않으므로, 디렉터리당 직렬 게이트(같은 폴더에서 한 번에 하나의 작업)가 서로 다른 두 이슈의 에이전트가 동시에 같은 파일에 쓰는 것을 막는 유일한 장치입니다. 그래서 "`local_directory`에 워크트리를 도입해 병렬 실행을 지원한다"는 말은 그 자체로 모순입니다 — 워크트리란 작업 트리를 *한 벌 더 복사하는 것*이고, 복사 비용을 감당할 수 없어서 이 모드를 선택한 것이기 때문입니다. **같은 코드베이스에서 진짜 병렬성이 필요하다면, 답은 `github_repo`입니다.**

### 로컬 디렉터리 연결하기

<Callout type="info">
시작하기 전에 정말로 이 모드가 필요한지 확인하세요(위 섹션 참고). 해당 폴더가 평범하게 클론할 수 있는 git 저장소라면 `github_repo`가 더 나은 선택입니다 — 동시성에 제한이 없습니다. `local_directory`가 겨냥하는 것은 수십 기가바이트짜리 게임 프로젝트처럼 한 번 클론하는 것조차 비용이 너무 큰 경우입니다.
</Callout>
...
블랙리스트는 의도적으로 공격적입니다 — 홈 디렉터리를 선택하면 CyberAgent의 런타임 파일이 계정의 루트에 놓이게 되는데, 이는 절대 원하는 결과가 아닙니다. 대신 하위 폴더(보통은 실제 프로젝트 체크아웃)를 선택하세요.
...
4. CyberAgent의 런타임 산출물(`output/`, `logs/`, `.gc_meta.json`)은 사용자의 디렉터리 **바깥**의 별도 envRoot에 둡니다.
...
### CyberAgent가 당신의 디렉터리에서 건드리는 것과 건드리지 않는 것
