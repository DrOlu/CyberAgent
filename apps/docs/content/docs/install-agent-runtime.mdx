...
1. **The CyberAgent daemon must be running.** Either run `multica daemon start` after installing the [CyberAgent CLI](/cli), or use the [Multica desktop app](/desktop-app), which launches the daemon automatically. Without a running daemon there is nothing to detect tools.
