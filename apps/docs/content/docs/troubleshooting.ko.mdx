---
title: 문제 해결
description: CyberAgent를 자체 호스팅할 때 흔히 겪는 문제 — 증상, 원인, 진단 방법, 해결 방법.
---
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | relay의 receive connector가 여러분의 서브넷이 외부 수신자에게 중계하는 것을 허용하지 않음(외부 도메인과 통신하는 익명 내부 relay에서 가장 흔함) | 초대를 내부 수신자로 제한하거나, CyberAgent 호스트의 서브넷을 Exchange "Anonymous Users → Relay" 권한 목록에 추가하세요 |
...
- `Unable to relay` → 내부 수신자로 제한하거나, Exchange receive connector에서 CyberAgent 호스트의 IP에 중계 권한을 부여하세요
...
2. **`pg_cron`이 설치되었지만 잘못된 데이터베이스를 가리킴** — `pg_cron.database_name`의 기본값은 `postgres`입니다. CyberAgent 데이터베이스 이름이 다르면 스케줄된 작업이 `rollup_task_usage_hourly()`를 전혀 보지 못합니다.
...
| **데몬(크래시)** | `~/.multica/daemon.err.log` | 직접 열기 — 패닉/초기 시작 오류가 여기에 기록됩니다 |
...
백그라운드 모드에서 `daemon.log`는 크기 기준으로 로테이션되므로(기본값 20MB × gzip 백업 5개; `MULTICA_DAEMON_LOG_MAX_SIZE_MB` / `_MAX_BACKUPS` / `_MAX_AGE_DAYS`로 조정 가능) 열 수 없을 만큼 커지지 않습니다. Go 런타임 패닉과 logger 초기화 이전 오류는 별도의 상한이 있는 `daemon.err.log`에 기록됩니다.

더 자세한 데몬 로그가 필요하면, 데몬을 백그라운드에서 포그라운드로 옮기세요: `multica daemon stop && multica daemon start --foreground`. 포그라운드로 실행하면 로그가 로테이션 파일이 아니라 터미널에 실시간으로 출력됩니다.
