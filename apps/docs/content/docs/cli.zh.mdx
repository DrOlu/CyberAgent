---
title: CLI 命令速查
description: CyberAgent CLI 的所有顶级命令一页概览。完整用法查 `multica <命令> --help`。
---

import { Callout } from "fumadocs-ui/components/callout";

CyberAgent CLI 把 Web UI 能做的事几乎全部搬到了命令行上（创建 [issue](/issues)、分配 [智能体](/agents)、启动 [守护进程](/daemon-runtimes) 等等）。这一页把所有顶级命令列出来，每条配一句用途。完整 flag 和示例用 `multica <命令> --help` 查。
...
| `multica setup cloud` | CyberAgent Cloud 一键初始化（登录 + 装 daemon） |
...
| `multica workspace create --name "..." --slug "..." [--issue-prefix "..."] [--description "..."] [--context "..."]` | 创建工作区（你会成为 owner）。`--name` 和 `--slug` 必填，slug 创建后不可修改；不会切换你当前的工作区。 |
