...
- **フロントエンドとバックエンドが別のホストにある場合は任意ではなく必須です**: ブラウザがページ自身の origin から `multica_csrf` cookie を読める必要があり、読み取りは動作したまま書き込みリクエストがすべて `403 CSRF validation failed` で失敗します
- 両方のホストを含む**最も狭い**親ドメインを使ってください（`.example.com` ではなく `.agent.example.com`）。この値は `multica_auth` セッション cookie もそのドメイン配下のすべてのホストに広げるため、それらがすべて同一の信頼できる主体によって運用されている場合にのみこの構成を使ってください — 詳細な設定は [Self-Hosting](/getting-started/self-hosting) を参照してください
