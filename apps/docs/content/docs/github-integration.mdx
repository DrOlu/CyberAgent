...
| **Webhook (background)** | On every `pull_request` event, CyberAgent upserts the PR row, scans the PR for issue identifiers, and (re)builds the link rows. Idempotent — replaying a delivery is a no-op. |
...
- **PRs in repos the App isn't installed on** — without the App, CyberAgent never receives the webhook.
- **Manually linking a PR to an issue** — there is no UI for this yet. If your team's convention puts identifiers in a place CyberAgent isn't reading, add them to the PR title or body.

## Disconnecting

In **Settings → GitHub** there is no installation list — you manage existing installations from GitHub directly:

- **From GitHub** — uninstall the CyberAgent GitHub App at `https://github.com/settings/installations` (personal) or `https://github.com/organizations/<org>/settings/installations` (org). CyberAgent receives the `installation.deleted` webhook and drops the row in real time; any open Settings tab updates without a refresh.
- **Disconnect from inside CyberAgent is admin-only** — the Disconnect control on the GitHub tab is hidden for non-admins. It stays available even when the master GitHub switch is off, so admins can still revoke a stale installation after one-click-disabling the feature.
...
- The GitHub App requests **read-only** access to pull requests and metadata. CyberAgent never pushes commits, comments, or status checks back to GitHub.

## Self-host setup

If you're running CyberAgent on CyberAgent Cloud, the integration is already configured — skip this section.
...
| Field | Value |
|---|---|
| **GitHub App name** | Anything recognizable, e.g. `CyberAgent` or `CyberAgent (staging)`. |
| **Homepage URL** | Your CyberAgent frontend, e.g. `https://multica.example.com`. |
| **Callback URL** | Leave blank — CyberAgent doesn't use OAuth user identity. |
| **Setup URL** | `https://<api-host>/api/github/setup`. **Check "Redirect on update"**. |
| **Webhook → Active** | Enabled. |
| **Webhook URL** | `https://<api-host>/api/webhooks/github`. |
| **Webhook secret** | Generate a long random string (e.g. `openssl rand -hex 32`). You'll paste the same value into CyberAgent's env in step 2. |
...
- The `/api/webhooks/github` endpoint returns **`503 github webhooks not configured`** — CyberAgent refuses to process events with no secret, rather than silently treating every signature as valid.
...
- **No CI / check state** — only the PR itself is mirrored. Build status, review comments, and reviewers are not surfaced in CyberAgent.
