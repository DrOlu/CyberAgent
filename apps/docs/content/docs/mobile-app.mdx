...
- The CyberAgent source code checked out:
...
There is no auto-update yet. When the CyberAgent codebase moves forward, pull and rebuild:
...
The iOS app is still moving fast — the team prefers ship-and-iterate over App Store review cycles right now. A TestFlight beta is the most likely next step before a full App Store release. Until then, the self-build path above is the only way to use CyberAgent on iOS.
