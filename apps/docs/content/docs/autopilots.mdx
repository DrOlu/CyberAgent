...
The CyberAgent server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; CyberAgent generates a unique URL of the shape:

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — CyberAgent records a run with `source = webhook`,
...
…or any JSON object/array. CyberAgent normalizes it into an internal envelope:
...
When you don't provide an `event` field, CyberAgent infers it from common
...
comma-separated **actions** list. CyberAgent allows a webhook if **any**
...
CyberAgent derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, CyberAgent reads the
...
present, CyberAgent falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, CyberAgent
...
CyberAgent returns `200 OK` with a `status` field for normal no-op outcomes so
...
custom self-hosted reverse proxies. CyberAgent deliberately does not derive
