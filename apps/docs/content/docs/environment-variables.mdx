...
description: The full list of environment variables for running a self-hosted CyberAgent server.
---

import { Callout } from "fumadocs-ui/components/callout";

A self-hosted CyberAgent [server](/self-host-quickstart) reads its configuration from environment variables at startup — database, sign-in, email, storage, signup allowlists all live here. This page groups every variable by purpose: each section spells out **what happens if you leave it unset** and **which ones you must set in production**. For how to actually configure the auth-related ones, see [Sign-in and signup configuration](/auth-setup).
...
CyberAgent supports two delivery backends — [Resend](https://resend.com/) for cloud deployments, or an SMTP relay for internal / on-premise networks. `SMTP_HOST` takes priority over `RESEND_API_KEY` when both are set.
...
CyberAgent stores user-uploaded attachments (images and files in comments). **S3 is preferred**; if S3 is not configured, it falls back to local disk.
...
- **Required, not optional, when the frontend and backend are on different hostnames**: the browser has to read the `multica_csrf` cookie from the page's own origin, and without it every write request fails with `403 CSRF validation failed` while reads keep working
- Use the **narrowest** parent domain that still covers both hosts (`.agent.example.com`, not `.example.com`). It also scopes the `multica_auth` session cookie to every host under that domain, so only use this layout when they are all operated by the same trusted party — see [Self-Hosting](/getting-started/self-hosting) for the full configuration
...
- The `/api/webhooks/github` endpoint returns **`503 github webhooks not configured`** — CyberAgent refuses to process events with no secret rather than treating every signature as valid.
...
By default, the server reports to CyberAgent's official PostHog instance. To opt out, set `ANALYTICS_DISABLED=true`.
