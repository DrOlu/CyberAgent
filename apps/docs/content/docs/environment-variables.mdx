...
CyberAgent uses [Resend](https://resend.com/) to send verification codes and invite emails.

| Variable | Default | Description |
|---|---|---|
| `RESEND_API_KEY` | empty | Resend API key |
| `RESEND_FROM_EMAIL` | `noreply@cyberagent.sh` | Sender address (must be a domain verified in your Resend account) |
...
| Variable | Default | Description |
|---|---|---|
| `FRONTEND_ORIGIN` | empty | Frontend address. Invite email links, the CORS allowlist, and the cookie domain are all derived from this. When unset, invite email links fall back to the hosted domain `https://app.cyberagent.sh` — self-host must set this explicitly |
