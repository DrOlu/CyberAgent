...
把 [issue](/issues) 分配给 [智能体](/agents)，它会作为**正式负责人**一直工作到结束——能读到 issue 的完整上下文（描述 + 所有 [评论](/comments)），也能改状态、发评论、改字段。这是 CyberAgent 四种触发方式里**最常见也最"重"**的一种。同样的流程也接受 [小队（squad）](/squads) 作为 assignee——这种情况下 CyberAgent 会触发小队的**队长智能体**。
...
非 Backlog 的 issue 分配给智能体之后，CyberAgent 会立刻在后台做以下事情：
...
当 assignee 是 [小队](/squads) 时，同样的状态约定适用：Multica 入队的是**队长**，队长首次接单时应把父 issue 推到 `in_progress`，并在成员干活期间保持该状态。只有整体目标达成后，队长才把父 issue 推到 `in_review`——第一次成功分发不等于完成。

**如果智能体离线**，`task` 会在队列里等——**5 分钟没被领走就超时失败**，失败原因 `runtime_offline`。对可重试的来源（分配、@ 提及、对话），CyberAgent 会自动重新排队；完整重试规则见 [**执行任务**](/tasks)。

分配还会自动把这个智能体加进 issue 的订阅列表——但 CyberAgent 里**智能体不接收 inbox 通知**（只有成员收）。这个订阅只是内部 bookkeeping，用户侧没有可见的副作用。
