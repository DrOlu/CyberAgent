...
description: Agents don't run on CyberAgent's servers — they run on your own machines.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

In CyberAgent, [agents](/agents) do **not** run on our servers — they run on your own machines, driven by a small program called the **daemon** that invokes the [AI coding tools](/providers) installed locally. The CyberAgent server only coordinates: it stores [issues](/issues), queues [tasks](/tasks), and dispatches them to the right **runtime** (runtime = daemon × one AI coding tool).

This structure is the biggest difference between CyberAgent and Linear / Jira: **your API keys, toolchain, and code directories stay on your machine** — the CyberAgent server never sees any of them. That means "my agent isn't working" is almost always a local problem — the daemon isn't running, an AI tool isn't installed, a key has expired. Check locally first; see [Troubleshooting](/troubleshooting) for a guide.

## Starting the daemon

The daemon is part of the CyberAgent CLI. Once you've installed the [CyberAgent CLI](/cli), run on your own machine:
...
A runtime is not a server and not a container — it's the combination of "**daemon × one AI coding tool**". For example: you start the daemon on a MacBook with both Claude Code and Codex installed, and you're a member of two workspaces. CyberAgent then registers 4 runtimes:
...
- The **Runtimes** page in the CyberAgent UI lists these rows

<Callout type="info">
**Cloud runtimes are coming**, currently in a waitlist phase. Once available, you'll be able to execute agent tasks directly on CyberAgent Cloud without running a local daemon. Sign up with your email on the [download page](https://cyberagent.sh/download) to get notified.
</Callout>

## When a runtime is marked offline

CyberAgent uses heartbeats to decide whether a runtime is online. Three key numbers:
...
**Tasks running on a missing runtime are marked as failed** (failure reason `runtime_offline`). For retryable sources (issues, chat), CyberAgent automatically requeues them; Autopilot-triggered tasks are not retried automatically. See [Tasks → Which failures retry automatically](/tasks#which-failures-retry-automatically-which-dont).
</Callout>

## How many tasks can run in parallel

CyberAgent enforces concurrency limits at two layers:
...
3. Open the **Runtimes** page in the CyberAgent UI — confirm your runtime shows "online"
