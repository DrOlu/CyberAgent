---
title: トラブルシューティング
description: CyberAgent をセルフホストする際によく遭遇する問題 — 症状、原因、診断方法、解決方法。
---
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | relay の receive connector が、あなたのサブネットから外部の受信者への中継を許可していない（外部ドメインと通信する匿名内部 relay で最も多い） | 招待を内部の受信者に制限するか、CyberAgent ホストのサブネットを Exchange の「Anonymous Users → Relay」権限リストに追加してください |
...
- `Unable to relay` → 内部の受信者に制限するか、Exchange の receive connector で CyberAgent ホストの IP に中継権限を付与してください
...
2. **`pg_cron` はインストールされているが誤ったデータベースを指している** — `pg_cron.database_name` のデフォルト値は `postgres` です。CyberAgent のデータベース名が異なる場合、スケジュールされたジョブは `rollup_task_usage_hourly()` を一切見つけられません。
...
| **デーモン（クラッシュ）** | `~/.multica/daemon.err.log` | 直接開く——パニックや起動初期のエラーはここに記録されます |
...
バックグラウンドモードでは `daemon.log` はサイズでローテーションされ（デフォルトは 20MB × gzip バックアップ 5 世代。`MULTICA_DAEMON_LOG_MAX_SIZE_MB` / `_MAX_BACKUPS` / `_MAX_AGE_DAYS` で調整可能）、開けないほど大きくなることはありません。Go ランタイムのパニックや logger 初期化前のエラーは、独立した上限付きの `daemon.err.log` に出力されます。

より詳細なデーモンログが必要な場合は、デーモンをバックグラウンドからフォアグラウンドに移してください: `multica daemon stop && multica daemon start --foreground`。フォアグラウンドで実行すると、ログはローテーションファイルではなくターミナルにリアルタイムで出力されます。
