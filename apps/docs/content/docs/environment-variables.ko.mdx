...
- **프런트엔드와 백엔드가 서로 다른 호스트에 있으면 선택이 아니라 필수입니다**: 브라우저가 페이지 자체 origin에서 `multica_csrf` cookie를 읽을 수 있어야 하며, 그렇지 않으면 읽기는 계속 동작하는 반면 모든 쓰기 요청이 `403 CSRF validation failed`로 실패합니다
- 두 호스트를 모두 포함하는 **가장 좁은** 상위 도메인을 사용하세요 (`.example.com`이 아니라 `.agent.example.com`). 이 값은 `multica_auth` 세션 cookie도 해당 도메인 아래의 모든 호스트로 확장하므로, 그 호스트들이 모두 동일한 신뢰 주체가 운영하는 경우에만 이 구성을 사용하세요 — 전체 설정은 [Self-Hosting](/getting-started/self-hosting)을 참고하세요
