...
Commencez par déterminer à quelle couche appartient le problème : le service CyberAgent, le daemon, le runtime ou l'outil de codage IA. Ces commandes suffisent généralement à faire apparaître la première erreur significative :
...
- Le daemon est connecté au mauvais service CyberAgent.
...
Ce statut (affiché « En attente du répertoire local ») signifie qu'une autre exécution en cours utilise le même répertoire local. CyberAgent attend que le verrou du répertoire soit libéré, afin que deux agents ne modifient jamais les mêmes fichiers en même temps.
...
## Les exécutions se bloquent sur macOS après une mise à jour du CLI

Après la mise à jour du CLI `multica` installé avec Homebrew, par `brew upgrade`, `multica update`, **Mettre à jour** sur la page **Runtimes** ou la mise à jour automatique du daemon, une exécution qui lit `~/Desktop`, `~/Documents`, `~/Downloads`, iCloud Drive ou un autre dossier protégé peut cesser de progresser : le processus de l'agent n'utilise plus de CPU, et l'exécution ne se termine ni n'échoue.

Cela se produit lorsque macOS attribue les autorisations de confidentialité (TCC) au daemon lui-même, comme pour un daemon lancé par launchd (par exemple un LaunchAgent) : la boîte de dialogue en attente affiche `multica`, et chaque outil lancé par un agent utilise les autorisations du daemon. Le binaire du CLI Homebrew n'est pas signé avec une identité stable : macOS lie donc ces autorisations à ce build précis et à son chemin résolu, par exemple `/opt/homebrew/Cellar/multica/<version>/bin/multica`. Après une mise à jour, macOS considère le nouveau binaire comme une autre application et redemande l'autorisation. L'accès au fichier reste bloqué jusqu'à ce que quelqu'un réponde, et le daemon ne peut pas savoir qu'une demande est en attente.
...
3. Redémarrez le daemon de la même façon que vous l'avez lancé : `multica daemon restart`, ou `launchctl kickstart -k gui/$(id -u)/<label>` pour un LaunchAgent.

Répétez ces étapes après chaque mise à jour du CLI. La mise à jour automatique n'est activée par défaut que pour les daemons connectés à Multica Cloud ; démarrez le daemon avec `--no-auto-update` pour que les mises à jour n'aient lieu que lorsque vous les lancez. Le CLI intégré à [Multica Desktop](/desktop-app) est signé avec un Developer ID, ses autorisations survivent donc aux mises à jour.
...
La connexion TCP s'ouvre, mais la poignée de main TLS n'aboutit jamais. Les clients Go, dont le CLI et le daemon CyberAgent, envoient par défaut un partage de clé post-quantique dans le ClientHello TLS, ce qui porte sa taille à environ 1,5 Ko et le répartit sur deux paquets. Certains logiciels de sécurité, VPN, routeurs et pare-feu abandonnent ces poignées de main ; curl envoie un ClientHello beaucoup plus petit et passe.
...
Lorsque `SMTP_HOST` et Resend sont tous deux configurés, CyberAgent privilégie SMTP. Pour la configuration, consultez [Connexion et inscription](/auth-setup#codes-de-vérification-par-e-mail).
...
S'il s'agit d'une autre copie de travail (checkout) de CyberAgent, exécutez d'abord `make stop` dans ce répertoire. Sinon, arrêtez normalement le programme en cause, ou changez le port du service actuel. Les ports publics 80/443 sont écoutés par un proxy inverse tel que Caddy ou Nginx.
