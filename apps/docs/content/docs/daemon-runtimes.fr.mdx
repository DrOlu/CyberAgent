...
description: Comment CyberAgent connecte les ordinateurs, détecte les outils de codage IA et lance les exécutions.
---

import { Callout } from "fumadocs-ui/components/callout";

CyberAgent enregistre et coordonne le travail ; ce sont les ordinateurs connectés qui l'exécutent. Le **daemon** d'un ordinateur prend en charge les exécutions et invoque les outils de codage IA installés sur cette machine.

## Daemon et runtime

- Le **daemon** est le processus d'arrière-plan de CyberAgent qui tourne sur un ordinateur. Il se connecte au serveur, détecte les outils locaux, prend en charge les exécutions et renvoie les résultats.
...
Les outils de codage IA qu'invoque un runtime local, leurs propres identifiants de connexion et vos répertoires de code locaux restent tous sur l'ordinateur connecté. Le serveur CyberAgent n'exécute pas de commandes à la place des outils locaux et ne téléverse pas automatiquement l'intégralité de votre répertoire de travail.
...
Avec CyberAgent Desktop, l'application démarre le daemon automatiquement — aucune commande supplémentaire n'est nécessaire.

Sur le web, sur un ordinateur distant ou dans un environnement sans interface graphique, [installez d'abord le CLI CyberAgent](/cli), puis exécutez :
...
| `MULTICA_SERVER_URL` | URL du serveur CyberAgent sélectionnée par le daemon | **Contrat d'intégration** |
| `MULTICA_TASK_CONFIG_ROOT` | Racine privée de configuration du CLI CyberAgent, propre à l'exécution | Indicatif |
...
Tout ce que vous saisissez dans le champ de commande reste directement après l'exécutable, avant les arguments ajoutés par CyberAgent :

```text
<your command> <your fixed arguments> <CyberAgent's protocol arguments> <the agent's custom arguments>
```

C'est ce qui permet à un wrapper à sous-commandes de fonctionner. Si votre commande est `ccms start q36`, l'outil voit d'abord `start q36` et peut sélectionner sa sous-commande avant que n'arrivent le `-p` de CyberAgent et le reste — c'est le seul ordre qu'accepte ce type de wrapper.
...
4. Ouvrez la page **Runtimes** de CyberAgent et vérifiez que l'ordinateur cible et l'outil de codage IA correspondant apparaissent en ligne.
