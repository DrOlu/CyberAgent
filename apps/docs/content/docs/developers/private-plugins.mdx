...
Private Skill Plugins let a workspace add declarative Agent guidance without publishing a new CyberAgent release. V1 accepts static UTF-8 text for `agent.skill.v1`; it is not an npm, Node, Python, WASM, or native extension system, and CyberAgent never runs package-manager or installation hooks from a Plugin.
