---
title: プロジェクトアーキテクチャ
description: CyberAgent のクライアント、Go サービス、実行デーモン、共有フロントエンドパッケージがどのように連携するかを説明します。
---

CyberAgent は Go バックエンド、複数のクライアント、実行用コンピューター上で動作するデーモンで構成されています。PostgreSQL がコラボレーションデータを保存し、デーモンがタスクを取得してローカルの AI コーディングツールを呼び出します。
...
プロダクト利用者向けの説明は [CyberAgent の仕組み](/how-multica-works)を参照してください。このページではコードの階層構造に焦点を当てます。
...
CyberAgent には異なる 2 つの WebSocket パスがあります。
