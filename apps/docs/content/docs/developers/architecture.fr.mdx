...
description: Découvrez comment les clients de CyberAgent, le service Go, le daemon d'exécution et les packages frontend partagés fonctionnent ensemble.
---

CyberAgent se compose d'un backend Go, de plusieurs clients et d'un daemon qui tourne sur les ordinateurs d'exécution. PostgreSQL stocke les données de collaboration ; le daemon récupère les tasks et invoque les outils de codage IA locaux.
...
Pour une explication orientée produit, consultez [Fonctionnement de CyberAgent](/how-multica-works). Cette page se concentre sur l'organisation du code en couches.
...
CyberAgent dispose de deux chemins WebSocket distincts :
