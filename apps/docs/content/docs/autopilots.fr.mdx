...
Chaque automatisation enregistre une procédure, un assigné et un ou plusieurs déclencheurs. Lorsqu'elle est déclenchée, CyberAgent crée une tâche ou exécute directement l'agent, et conserve une trace de chaque exécution.
...
Après l'ajout d'un déclencheur webhook, CyberAgent génère une URL unique. Envoyez-lui un objet ou un tableau JSON pour déclencher l'automatisation :
...
- sans clé d'idempotence stable, CyberAgent ne peut pas garantir que des requêtes répétées ne s'exécutent qu'une seule fois ;
...
Par exemple, avec `workflow_run` comme nom d'événement et `completed, failed` comme actions, seuls ces deux types de résultats `workflow_run` sont acceptés. CyberAgent reconnaît l'événement et l'action à partir des en-têtes de requête et des champs de charge utile courants, notamment l'en-tête `X-GitHub-Event` de GitHub et le champ `action` du corps.
...
Lors du débogage d'un expéditeur, utilisez ce tableau pour interpréter les réponses de CyberAgent :
...
| 500 | Message d'erreur | Erreur interne de CyberAgent ; l'expéditeur peut réessayer plus tard. |
