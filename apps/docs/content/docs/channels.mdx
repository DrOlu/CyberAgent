...
4. **Identity + authorization** — resolve the sender's platform user id to a CyberAgent user (the [account binding](#authorization)), then re-check workspace membership. Unbound senders get a "link your account" prompt; non-members are dropped.
...
- **Account binding (authentication)** — the sender's platform user id must be linked to a CyberAgent user. The first time someone messages the bot they get a one-time link to bind their identity to **their own** CyberAgent account; until then no agent runs.
- **Workspace membership (authorization)** — the bound CyberAgent user must be a member of the installation's workspace, re-checked on every message. Non-members are silently dropped.
...
On CyberAgent Cloud both are already configured. See [Environment variables](/environment-variables) for the full reference.
