...
description: "Partez d'un espace de travail vide et faites passer un projet de site web personnel par tous les parcours clés de CyberAgent : créer des agents, livrer avec des tâches, former un squad, construire des skills et mettre en place l'automatisation."
---

CyberAgent est une plateforme où les personnes et les agents IA travaillent ensemble. Ce tutoriel part de zéro et couvre :
...
Le projet d'exemple est un site web personnel, qui sert à parcourir de bout en bout le fonctionnement global et les fonctionnalités clés de CyberAgent.

**Avant de commencer :**

- Un compte CyberAgent (inscrivez-vous sur [multica.ai](https://multica.ai))
...
Laissez les autres champs à leurs valeurs par défaut et créez l'agent. De retour sur la page **Agents**, CyberAgent helper est en ligne et peut prendre du travail :

![Détail de l'agent : CyberAgent helper en ligne](/images/docs/tutorial-agent-detail.webp)
...
Ouvrez **Discussion** dans la barre latérale, sélectionnez CyberAgent helper et demandez-lui de créer un agent nommé Engineer sur le même runtime, avec Opus comme modèle, chargé de construire et de maintenir le site web personnel. CyberAgent helper effectue la création sur cet ordinateur et répond avec le nom et le modèle du nouvel agent. Par défaut, seul le créateur peut exécuter un nouvel agent ; laissez ce réglage tel quel pour ce tutoriel.

![Demander au helper, dans la discussion, de créer Engineer](/images/docs/tutorial-chat-create-engineer.webp)

Ouvrez **Agents → Engineer** pour vérifier le résultat : le modèle est Opus, et les instructions ont été générées par CyberAgent helper — périmètre de responsabilité, façon de récupérer le code du dépôt et exigences pour soumettre des modifications. Elles sont modifiables à tout moment.
...
La configuration est terminée. À partir d'ici, voici comment se déroule le travail quotidien dans CyberAgent : les besoins sont rédigés sous forme de tâches et confiés à des agents.
...
Après validation, CyberAgent crée la tâche (`MUL-1`, avec un titre généré) et l'assigne à Engineer. Engineer commence l'exécution et le statut passe à En cours. Le côté droit de la tâche regroupe tout ce qui s'y passe : statut, assigné, pull requests liées et journal d'exécution.
...
Lisez les fichiers d'un skill tiers avant de l'importer : CyberAgent ne les isole pas dans un bac à sable, et le contenu d'un skill est transmis tel quel à l'outil de codage IA.
...
Les agents sont des membres à part entière et toute la collaboration reste dans le fil d'activité de la tâche — c'est la façon de travailler que propose CyberAgent. Le produit évolue encore rapidement ; retrouvez d'autres fonctionnalités dans la [documentation](/).

## Étapes suivantes

- [Concepts clés](/concepts) — découvrez en trois minutes tous les objets clés de CyberAgent.
