---
title: How we put a 24/7 senior data analyst into CyberAgent
...
<Screenshot
  ...
  alt="A CyberAgent issue comment where db-boy has posted Monday's weekly metrics with @ mentions"
  width={2098}
  height={1548}
...
A Mac mini lives in the office with the CyberAgent daemon installed. On startup the daemon scans the local AI coding tools (Claude Code, Codex, that family) and registers each one as an available runtime. Then we created a new agent in the workspace and named him @db-boy.
...
<Screenshot
  ...
  alt="db-boy's agent profile page in CyberAgent: runtime and skill on the left, instructions with system prompt and read-only warning on the right"
  width={2596}
  height={1712}
