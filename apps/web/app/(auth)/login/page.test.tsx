...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to CyberAgent")).toBeInTheDocument();
  ...
  it("calls sendCode with email on submit", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "test@cyberagent.sh");
    await user.click(screen.getByRole("button", { name: "Continue" }));

    await waitFor(() => {
      expect(mockSendCode).toHaveBeenCalledWith("test@cyberagent.sh");
    });
  });
  ...
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@cyberagent.sh" };
    ...
    try {
      ...
      expect(
        await screen.findByRole("button", { name: "Open CyberAgent Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
